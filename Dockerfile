...
RUN set -ex \
    ...
    && apt-get install -yqq --no-install-recommends $buildDeps \
        freetds-bin build-essential default-libmysqlclient-dev \
    ...
    && pip install apache-airflow[crypto,postgres,jdbc${AIRFLOW_DEPS:+,}${AIRFLOW_DEPS}]==${AIRFLOW_VERSION} \
